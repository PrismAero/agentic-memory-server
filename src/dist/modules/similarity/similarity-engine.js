...
export class ModernSimilarityEngine {
    ...
    async initialize() {
        try {
            logger.info("Initialized modern similarity engine with sentence-similarity + natural");
    ...
    async detectSimilarEntities(targetEntity, candidateEntities) {
        ...
        try {
            logger.debug(`Modern similarity engine analyzing ${candidateEntities.length} candidates for "${targetEntity.name}"`);
            const results = await this.relationshipDetector.detectSimilarEntities(targetEntity, candidateEntities);
            logger.debug(`Found ${results.length} similar entities above threshold`);
            // Log detailed results for debugging
            results.forEach((result, i) => {
                logger.debug(`${i + 1}. "${result.entity.name}" - ${(result.similarity * 100).toFixed(1)}% similar (${result.confidence}) - ${result.suggestedRelationType}`);
            });
            return results;
        }
        catch (error) {
    ...
    async calculateSimilarity(entity1, entity2) {
        ...
        const results = await this.relationshipDetector.detectSimilarEntities(entity1, [entity2]);
        return results.length > 0 ? results[0].similarity : 0;
    }
    /**
     * Batch similarity calculation for performance
     */
    async calculateBatchSimilarity(entities) {
        if (!this.initialized) {
            await this.initialize();
        }
        ...
        for (const entity of entities) {
            ...
            const similarities = await this.relationshipDetector.detectSimilarEntities(entity, otherEntities);
            results.set(entity.name, similarities.map((s) => ({
                entity: s.entity,
