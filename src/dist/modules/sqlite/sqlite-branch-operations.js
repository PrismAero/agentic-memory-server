...
export class SQLiteBranchOperations {
    ...
    async createBranch(branchName, purpose) {
        await this.connection.execQuery("INSERT INTO memory_branches (name, purpose) VALUES (?, ?)", [branchName, purpose || `Custom branch: ${branchName}`]);
        return {
            name: branchName,
    ...
    async deleteBranch(branchName) {
        ...
        await this.connection.execQuery("DELETE FROM memory_branches WHERE id = ?", [branchId]);
    }
    async listBranches() {
    ...
    async suggestBranch(entityType, content) {
        ...
        for (const branch of branches) {
            ...
            if (searchTerms.some((term) => ["doc", "documentation", "spec", "guide"].includes(term)) &&
                (branchName.includes("doc") || branchPurpose.includes("doc"))) {
                score += 8;
            }
            if (searchTerms.some((term) => ["demo", "example", "sample", "test"].includes(term)) &&
